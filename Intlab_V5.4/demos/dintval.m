...
% Two types of interval standard functions are available in INTLAB: 
% The built-in standard functions with a certain safety margin 
% added (see startup file), and rigorous standard functions.
% The first ones are faster than the rigorous ones, therefore
% they are still kept, but might be incorrect. 
%
% One may switch between the two possibilities by "intvalinit". 
% For example
      
x = intval(1e10); format long
intvalinit('ApproximateStdFcts'), sin(x)
intvalinit('RigorousStdFcts'), sin(x)
          
%%
% Note that the second result is rigorous (try sin(2^1000) or similar).
% For timing comparison of "approximate" interval standard functions
% and rigorous standard functions consider

n=10000; x=intval(random(n,1));
intvalinit('ApproximateStdFcts'), tic, exp(x); toc
intvalinit('RigorousStdFcts'), tic, exp(x); toc
