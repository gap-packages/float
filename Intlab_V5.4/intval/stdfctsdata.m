...
%
  ...
  INTLAB_INTVAL_STDFCTS_EPSMAX = 5e-16;
  success = 1;
  ...
  success = EstimateExpSinCosTanSinhTanh
  if ~success
    NoSuccess;
  end
  ...
  savedata(INTLAB_INTVAL_STDFCTS_SUCCESS)
...
%
  ...
  savedata(INTLAB_INTVAL_STDFCTS_ATAN)
...
%
  ...
  savedata(INTLAB_INTVAL_STDFCTS_LOG)
...
function success = EstimateExpSinCosTanSinhTanh
...
%
  ...
  savedata(INTLAB_INTVAL_STDFCTS_EXP)

  INTLAB_INTVAL_STDFCTS_SIN = E_SIN;
  savedata(INTLAB_INTVAL_STDFCTS_SIN)

  INTLAB_INTVAL_STDFCTS_COS = E_COS;
  savedata(INTLAB_INTVAL_STDFCTS_COS)

  INTLAB_INTVAL_STDFCTS_TAN = E_TAN;
  savedata(INTLAB_INTVAL_STDFCTS_TAN)

  INTLAB_INTVAL_STDFCTS_SINH = E_SINH;
  savedata(INTLAB_INTVAL_STDFCTS_SINH)

  INTLAB_INTVAL_STDFCTS_TANH = E_TANH;
  savedata(INTLAB_INTVAL_STDFCTS_TANH)
...
function savedata(DATA)
  ...
  eval([ inputname(1) ' = DATA' ])
  fname  = intvalinit('stdfctsfile');
  if exist(fname,'file')
    eval([ 'save ' fname ' ' inputname(1) ' -append' ])
  else
    eval([ 'save ' fname ' ' inputname(1) ])
  end
  disp(' ')
