...
%
  ...
  elseif ~isa(b,'hessian')      % hessian array * non-hessian array
    ...
    dims = m*n;
    r.dx = reshape(reshape(b.dx(:,reshape(1:k*n,k,n)'),N*n,k)*(a.x).',N,dims);
    r.dx = r.dx(:,reshape(1:n*m,n,m)') + reshape(reshape(a.dx,N*m,k)*(b.x),N,dims);
    e = reshape(b.dx.',k,n*N);
    r.hx = reshape(a.dx,m*N,k) * reshape(e(:,reshape(1:n*N,n,N)'),k,n*N);
    e = reshape(reshape(b.hx(:,reshape(1:k*n,k,n)'),N^2*n,k)*(a.x).',N^2,dims);
    r.hx = reshape(reshape(r.hx(:,reshape(1:n*N,N,n)'),m*n*N,N).',N^2,m*n) + ...
      e(:,reshape(1:n*m,n,m)') + reshape(reshape(a.hx,N^2*m,k)*(b.x),N^2,dims);
