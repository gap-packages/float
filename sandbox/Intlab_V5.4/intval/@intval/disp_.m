...
%
  ...
  if isequal(INTLAB_INTVAL_DISPLAY,'Display_')   % otherwise forced to use disp_
    if ( ~isempty(c) ) & mean(relerr(nonzeros(c)))>.02   % intervals inaccurate, change to other output
      if nargout
        evalstr = 'outstr = '
      else
        evalstr = '';
...
%AAAA Matlab V5.2 bug fix
  ...
  if ~isequal(format(end),'E')
    ...
    if c.complex
      ...
      cre(find(isinf(cre))) = 0;                % fast for sparse matrices
      cim(find(isinf(cim))) = 0;
      cremax = max(cre(:));
      cimmax = max(cim(:));
      ...
      cmax(find(isinf(cmax))) = 0;              % fast for sparse matrices
      cmax = max(cmax);                         % take care of linear indices for huge matrices
      cmax = max(cmax(:));
    end
