...
%AAAA Matlab V5.2 bug fix
  ...
  if ~isequal(format(end),'E')
    ...
    if c.complex
      creinf = abs(real(cinf));
      cresup = abs(real(csup));
      ciminf = abs(imag(cinf));
      cimsup = abs(imag(csup));
      creinf(find(isinf(creinf))) = 0;
      cresup(find(isinf(cresup))) = 0;
      ciminf(find(isinf(ciminf))) = 0;
      cimsup(find(isinf(cimsup))) = 0;
      maxcreinf = max(creinf(:));
      maxcresup = max(cresup(:));
      maxciminf = max(ciminf(:));
      maxcimsup = max(cimsup(:));
      cmax = max([ maxcreinf maxcresup maxciminf maxcimsup ]);
    else
      cinf_ = abs(real(cinf(:)));
      csup_ = abs(real(csup(:)));
      cinf_(find(isinf(cinf_))) = 0;
      csup_(find(isinf(csup_))) = 0;
      cinf_max = max(cinf_);
      csup_max = max(csup_);
