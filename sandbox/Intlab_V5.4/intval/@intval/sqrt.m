  ...
  if any(index(:))                       % exceptional arguments
    ...
    if any(index(:))              
      if INTLAB_INTVAL_STDFCTS           % rigorous standard function
        y.inf(index) = sqrt_rnd(x.inf(index),-1);
        y.sup(index) = sqrt_rnd(x.sup(index),1);
      else                               % approximate standard function
        global INTLAB_INTVAL_EPSSTDFCTS
        setround(0)
        y_ = sqrt(x.inf(index));
        y.inf(index) = y_ - INTLAB_INTVAL_EPSSTDFCTS*min(realmax,abs(y_));
        y_ = sqrt(x.sup(index));
        y.sup(index) = y_ + INTLAB_INTVAL_EPSSTDFCTS*min(realmax,abs(y_));
      end
    end
